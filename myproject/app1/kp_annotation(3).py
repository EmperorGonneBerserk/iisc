...
def display_parts_colors(app):
    part_colors_window = app.part_colors_window  # Access the part_colors_window from the app instance
    confidence_values = app.keypoint_conf  # Use the loaded confidence values

    def toggle_confidence(index):
        # Toggle the confidence value between 1.0 and 0.05
        confidence_values[index] = 0.05 if confidence_values[index] == 1.0 else 1.0
        # Update the button text
        button = buttons[index]  # Get the button reference from the list
        button.config(text=f"{parts[index]}: {'High' if confidence_values[index] == 1.0 else 'Low'}")
        # Update the confidence in the main application
        app.update_confidence(confidence_values)
    ...
    for i in range(len(parts)):
        ...
        # Create a button and store its reference in the buttons list
        button = Button(part_colors_window, text=f"{parts[i]}: {'High' if confidence_values[i] == 1.0 else 'Low'}", command=lambda index=i: toggle_confidence(index))
        button.grid(row=i, column=1, sticky="e")
        buttons.append(button)  # Append the button to the list
...
class KeypointEditor:
    def __init__(self, root,folder_path, model_type):
        ...
        self.draw_bbox()
        self.check_file()
        self.save_button = Button(root, text="Save(S)", command=self.save_coordinates)
        ...
        self.next_button = Button(root, text="Next(D)", command=self.next_image)
        self.prev_button = Button(root, text="Previous(A)", command=self.prev_image)
        self.delete_button = Button(root, text="Delete (W)", command=self.delete_current_image)
        self.pack_buttons()
    ...
    def open_part_colors(self):
        if self.part_colors_window is not None:
            self.part_colors_window.destroy()  # Close the existing window if it exists
        self.part_colors_window = Toplevel()
        self.part_colors_window.title("Keypoint Confidence")
        display_parts_colors(self)  # Call the function to display the parts and their confidence

    def update_confidence(self, confidence_values):
        self.keypoint_conf = confidence_values.copy()  # Use copy to avoid reference issues
    ...
    def load_annotation(self, image_path):
        ...
        if os.path.exists(annotation_path):
            ...
            self.final_points = []
            self.keypoint_conf = []  # Reset keypoint confidence
            self.initial_points = []
            for i in range(0, len(keypoints), 3):
                ...
                self.keypoint_conf.append(conf)  # Load confidence values
    ...
    def load_image(self):
        ...
        if self.is_initial:
            try:
                ...
                # Load annotations and set confidence values
                if self.load_annotation(image_path):
                    # If annotations are loaded, set the confidence values based on the loaded annotations
                    print("Annotations loaded.")
                    self.update_confidence(self.keypoint_conf)  # Update confidence values from loaded annotations
                else:
                    print("No annotations found, setting high confidence for all keypoints.")
                    results = model(image)
                    ...
                    self.is_initial = False
                    self.keypoint_conf = [1.0] * self.keypoint_count  # Set high confidence for all keypoints
                    self.update_confidence(self.keypoint_conf)  # Update the sidebar to reflect high confidence
        ...
        # Open the part colors window to reflect the current confidence values
        self.open_part_colors()
    ...
    def save_coordinates(self):
        ...
        for i, p in enumerate(self.final_points):
            normalized_points.append((p[0] / self.image_size, p[1] / self.image_size, self.keypoint_conf[i]))  # Use updated confidence values
    ...
    def move_selected_point(self, event):
        if self.selected_point is not None:
            self.selected_bbox = None
            x, y = self.convert_coordinates(event.x / self.scale_factor), self.convert_coordinates(event.y / self.scale_factor)
            self.final_points[self.selected_point] = [x, y]
            self.keypoint_conf[self.selected_point] = self.current_conf
            self.redraw_keypoints()  # Call to redraw keypoints and connections
        if self.selected_bbox is not None:
            self.selected_point = None
            x, y = self.convert_coordinates(event.x, True), self.convert_coordinates(event.y, True)
    ...
    def redraw_keypoints(self):
        ...
        self.draw_connections(self.connections)  # Redraw connections
        self.draw_keypoints()  # Redraw keypoints
        self.draw_bbox()  # Redraw bounding box
...
if __name__ == "__main__":
    try:
        ...
        parts_button = Button(root, text="Display Parts and Colors", command= display_parts_colors(app))
        parts_button.pack()
        root.mainloop()
    except Exception as e:
